 ...
 * <ul>
 *   <li>{@code 0} â€” species A (majority / host)</li>
 *   <li>{@code 1} â€” species B (first solute)</li>
 *   <li>{@code 2} â€” species C (second solute)</li>
 *   <li>â€¦and so on.</li>
 * </ul>
 * <p>For a <em>binary</em> system this reduces to A=0, B=1, and basis
 * function {@code Ï†_1(0) = +1}, {@code Ï†_1(1) = -1}, recovering the
 * standard Â±1 spin operator.</p>
 *
 * <h2>Orthogonal basis construction</h2>
 * <p>The basis functions {@code Ï†_Î±} for {@code Î± = 1, â€¦, numCompâˆ’1} are
 * chosen to be orthonormal with respect to the uniform (random-alloy) measure
 * on {@code {0, 1, â€¦, numCompâˆ’1}}.  The explicit construction used here is
 ...
 * <p>For a decorated cluster with sites labelled {@code s1, s2, â€¦} the cluster
 * product is:</p>
 * <pre>
 *   Î¦(cluster) = Î _{k in cluster} Ï†_{Î±(k)}(occ_k)
 * </pre>
 * <p>where {@code Î±(k)} is the basis index of site {@code k} (read from
 * the {@code "sÎ±"} symbol of the corresponding cluster orbit member),
 * and {@code occ_k âˆˆ {0,â€¦,numCompâˆ’1}} is the actual occupation of site
...
public class SiteOperatorBasis {
    ...
    /**
     * {@code basisMatrix[alpha][sigma]} = Ï†_{alpha+1}(sigma),
     * for {@code alpha âˆˆ [0, numCompâˆ’2]} and {@code sigma âˆˆ [0, numCompâˆ’1]}.
     */
     ...
     *
     * @param numComp number of chemical components; must be â‰¥ 2
     ...
     * {@code numComp âˆ’ 1}.
     *
     * @return {@code numComp âˆ’ 1} â‰¥ 1
     */
    public int getNumBasisFunctions() { return numComp - 1; }

    /**
     * Evaluates basis function {@code Ï†_alpha} at occupation {@code sigma}.
     *
     * <p>Basis index {@code alpha} is 1-based (matching the {@code "s1"},
     * {@code "s2"}, â€¦ naming convention used in decorated cluster sites).
     * Use {@code alpha = 1} for the dominant correlation function.</p>
     *
     * @param alpha basis-function index in {@code [1, numCompâˆ’1]}
     * @param sigma occupation in {@code [0, numCompâˆ’1]}
     * @return {@code Ï†_alpha(sigma)}
     ...
     *
     * <p>Recognises the convention used by {@link org.ce.domain.identification.engine.BasisSymbolGenerator}:
     * {@code "s1"} â†’ 1, {@code "s2"} â†’ 2, etc.</p>
     *
     * @param symbol site symbol, e.g. {@code "s1"}
     * @return parsed alpha index â‰¥ 1
    ...
    // -------------------------------------------------------------------------
    // Basis construction â€” Gram-Schmidt over uniform measure
    // -------------------------------------------------------------------------

    /**
     * Builds the {@code (numCompâˆ’1) Ã— numComp} basis matrix by Gram-Schmidt
     * orthogonalisation of the power basis {@code {1, Ïƒ, ÏƒÂ², â€¦}} over the
     * uniform measure on {@code {0, 1, â€¦, numCompâˆ’1}}.
     *
    ...
    private static double[][] buildBasis(int n) {
        // Power basis vectors: v_k(Ïƒ) = Ïƒ^k, k = 0, 1, ..., n-1
        ...
        // v_0 = constant (empty cluster / identity) â€” normalised to 1/sqrt(n)
        for (int s = 0; s < n; s++) ortho[0][s] = 1.0 / Math.sqrt(n);

        for (int k = 1; k < n; k++) {
            // Start with v_k(Ïƒ) = Ïƒ^k
            ...
            for (int j = 0; j < k; j++) {
                ...
                // uniform measure: inner product = (1/n) Î£ fÂ·g, but since
                // ortho[j] is normalised w.r.t. (1/n) measure, projection = dot/n * n = dot
                // Actually: inner product under (1/n) measure of f and g = (1/n) Î£ f(s)g(s)
                // projection of vk onto ortho[j] = <vk, ortho[j]> / <ortho[j], ortho[j]>
                //   = (1/n Î£ vk*ortho[j]) / 1  (since ortho[j] is already normalised)
                double proj = 0.0;
                for (int s = 0; s < n; s++) proj += vk[s] * ortho[j][s];
                proj /= n;  // (1/n) Î£ vk * ortho[j]
            ...
            // Cleaner Gram-Schmidt using unnormalised inner product Î£ fÂ·g / n
            // (restart for clarity)
            vk = new double[n];
            for (int s = 0; s < n; s++) vk[s] = Math.pow(s, k);
            ...
            for (int j = 0; j < k; j++) {
                double ip = 0.0;   // <vk, prev[j]> = (1/n) Î£ vk * prev[j]
            ...
            ortho[k]      = vk;
            basis[k - 1]  = vk;   // Ï†_k = ortho[k], alpha = k (1-based)
        }
    ...
    public void printDebug() {
        ...
        for (int a = 0; a < numComp - 1; a++) {
            StringBuilder sb = new StringBuilder(
                    String.format("  Ï†_%d : ", a + 1));
            for (int s = 0; s < numComp; s++) {
                sb.append(String.format("Ïƒ=%d: %+.3f  ", s, basisMatrix[a][s]));
            }
