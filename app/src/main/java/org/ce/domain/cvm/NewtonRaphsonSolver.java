 ...
 *
 * <p>Minimises {@code G(u) = H(u) âˆ’ TÂ·S(u)} over the non-point correlation
 * functions {@code u[0..ncf-1]} by iterating:</p>
 * <pre>
 *   Î´u = LinearSolve(Gcuu, âˆ’Gcu)
 *   u  â† u + step Â· Î´u
 * </pre>
 * until {@code ||Gcu|| < tolerance}.
 *
 * <p>Corresponds to Mathematica Section 10 "Internal Equilibrium (Gibbs Energy
 * Minimization)" â€” the manual Newton-Raphson method with step damping.</p>
 *
 * @see CVMFreeEnergy
...
public final class NewtonRaphsonSolver {
     ...
     * @param step          NR step damping factor (0 < step â‰¤ 1)
    ...
    public static CVMSolverResult solve(
        ...
        // For each non-point CF, the random value = Î  pointCF[basisIndex].
        // This ensures all CVs are positive from the start, which is
        // critical for Kâ‰¥3 where u=0 maps to a pure-component boundary.
        ...
        System.out.println("    â†’ CFs u[0.." + (ncf-1) + "] are OPTIMIZED");
        System.out.println("    â†’ CFs u[" + ncf + ".." + (tcf-1) + "] are FIXED (point/empty CFs)");
        ...
        for (iter = 1; iter <= maxIter; iter++) {
            ...
            if (gradNorm < tolerance) {
                System.out.println("  âœ“ CONVERGED");
                return new CVMSolverResult(u, G, H, S, iter, gradNorm, true);
            }

            // Newton-Raphson step: Î´u = solve(Gcuu, -Gcu)
            ...
            try {
                ...
                // Singular Hessian â€” cannot continue
                System.out.println("  âœ— SINGULAR HESSIAN");
            ...
            // strictly positive after updating u â† u + alphaÂ·Î´u.
            ...
            System.out.printf("      Step: Î±=%.6f  ||Î´u||=%.8e%n%n", alpha, norm(deltaU));
        }
     ...
     * {@code cv_new = cv_old + alpha * Î£_k cmat[v][k] * delta_full[k]}
        ...
        for (int t = 0; t < tcdis; t++) {
            for (int j = 0; j < lc[t]; j++) {
                ...
                for (int v = 0; v < nv; v++) {
                    ...
                    // Only constrain if CV is currently positive â€” for already-
                    // negative CVs (Kâ‰¥3 initial state), we let the solver move
