...
import org.ce.cvm.CVMFreeEnergy;
import org.ce.cvm.ClusterVariableEvaluator;
...
public class CVMModelInspectorDialog extends Dialog<Void> {
    ...
    private VBox createContent() {
        ...
        if (systemCombo.getItems().isEmpty()) {
            displayArea.setText(
                ...
                "Systems must have:\n" +
                "  ✓ Stage 1: Cluster Identification (HSP + ordered)\n" +
            ...
            displayArea.setText(
                ...
                "• Cluster identification results (Stage 1)\n" +
                "• Correlation function data (Stage 2)\n" +
                "• C-matrix dimensions (Stage 3)\n" +
                "• ECI requirements\n" +
                "• Validation test results\n" +
                "• Random limit entropy check\n\n" +
    ...
    private void inspectSelectedSystem() {
        ...
        try {
            ...
            if (data.isComplete()) {
                statusLabel.setText("✓ Model data complete and validated");
                statusLabel.setStyle("-fx-padding: 5; -fx-background-color: #d4edda; -fx-text-fill: #155724;");
            } else {
                statusLabel.setText("⚠ Model data incomplete: " + data.getFirstIncompleteStage());
                statusLabel.setStyle("-fx-padding: 5; -fx-background-color: #f8d7da; -fx-text-fill: #721c24;");
            }
    ...
    private String generateInspectionReport(SystemIdentity system, AllClusterData data) {
        ...
        sb.append("──────────────────────────────────────────────────────────────────────\n");
        sb.append(String.format("%-20s: %s\n", "Complete", data.isComplete() ? "✓ YES" : "✗ NO"));
        sb.append(String.format("%-20s: %s\n", "Stage 1", data.getStage1() != null ? "✓" : "✗ MISSING"));
        sb.append(String.format("%-20s: %s\n", "Stage 2", data.getStage2() != null ? "✓" : "✗ MISSING"));
        sb.append(String.format("%-20s: %s\n", "Stage 3", data.getStage3() != null ? "✓" : "✗ MISSING"));
        sb.append(String.format("%-20s: %d ms\n", "Computation Time", data.getComputationTimeMs()));
        sb.append("\n");
        
        if (!data.isComplete()) {
            sb.append("⚠ WARNING: Model data is incomplete. Missing: ").append(data.getFirstIncompleteStage()).append("\n");
        ...
        sb.append("  Format: cmat[HSP_type][group] = matrix[CV_rows × CF_cols]\n");
        ...
        for (int t = 0; t < cmat.size(); t++) {
            ...
            sb.append(String.format("  ┌─ HSP TYPE [%d]: %d group(s)\n", t, groups.size()));
            ...
            for (int j = 0; j < groups.size(); j++) {
                ...
                sb.append(String.format("  │\n"));
                sb.append(String.format("  ├── GROUP [%d]: %d CVs × %d columns (ncf=%d + 1 constant)\n", 
                         j, nrows, ncols, ncols - 1));
                
                // Show weights for this group
                List<int[]> wcvForType = wcv.get(t);
                int[] weights = wcvForType.get(j);
                sb.append(String.format("  │   Weights wcv[%d][%d]: [", t, j));
                ...
                int maxRowsToShow = (nrows <= 8) ? nrows : 6; // Show all if ≤8, else first 6
                int maxColsToShow = (ncols <= 8) ? ncols : 6; // Show all if ≤8, else first 6
                
                for (int r = 0; r < maxRowsToShow; r++) {
                    sb.append(String.format("  │     row[%d]: [", r));
        ...
        sb.append("──────────────────────────────────────────────────────────────────────\n");
        sb.append(String.format("%-30s: %s\n", "Dimension consistency", dimTest ? "✓ PASS" : "✗ FAIL"));
        sb.append(String.format("%-30s: %s\n", "KB coefficient check", kbTest ? "✓ PASS" : "✗ FAIL"));
        if (system.getNumComponents() == 2) {
            sb.append(String.format("%-30s: %s\n", "Random entropy test", entropyTest ? "✓ PASS" : "✗ FAIL"));
        }
    ...
    private boolean performDimensionTest(ClusterIdentificationResult stage1, 
        ...
        // Check: tcf = ncf + nxcf
        if (tcf != ncf + nxcf) {
            sb.append(String.format("  ✗ FAIL: tcf (%d) ≠ ncf (%d) + nxcf (%d) = %d\n", 
                     tcf, ncf, nxcf, ncf + nxcf));
            pass = false;
        } else {
            sb.append(String.format("  ✓ tcf = ncf + nxcf (%d = %d + %d)\n", tcf, ncf, nxcf));
        }
        
        // Check: tcdis should be reasonable
        if (tcdis < 2) {
            sb.append(String.format("  ✗ FAIL: tcdis (%d) too small\n", tcdis));
            pass = false;
        } else {
            sb.append(String.format("  ✓ tcdis = %d (reasonable)\n", tcdis));
        }
        
        // Check: tc <= tcdis * (some reasonable factor)
        if (tc > tcdis * 100) {
            sb.append(String.format("  ⚠ WARNING: tc (%d) >> tcdis (%d), unusually large\n", tc, tcdis));
        }
    ...
    private boolean performKBCoefficientTest(ClusterIdentificationResult stage1, StringBuilder sb) {
        ...
        if (Math.abs(kb[0] - 1.0) > 1e-10) {
            sb.append(String.format("  ✗ FAIL: kb[0] = %.10f (expected 1.0)\n", kb[0]));
            pass = false;
        } else {
            sb.append(String.format("  ✓ kb[0] = %.10f (maximal cluster)\n", kb[0]));
        }
        
        // Check: sum of kb should be non-negative (typically positive for point cluster contribution)
        double sum = 0.0;
        for (double k : kb) sum += k;
        sb.append(String.format("  Σ kb = %.10f\n", sum));
    ...
    private boolean performRandomEntropyTest(AllClusterData data, StringBuilder sb) {
        ...
        sb.append("  S_ideal = -R·Σ xi·ln(xi) = -R·(0.5·ln(0.5) + 0.5·ln(0.5))\n");
        sb.append("  S_ideal = R·ln(2) ≈ 0.693147·R\n\n");
        ...
        try {
            ...
            sb.append(String.format("    All u[l] ≈ 0? %s\n\n", allZero ? "✓ YES" : "✗ NO"));
            ...
            sb.append("      S = -R · Σ_t kb[t]·ms[t] · Σ_j mh[t][j] · Σ_v w[t][j][v] · Φ(cv[t][j][v])\n");
            sb.append("      Φ(cv) = cv·ln(cv) for cv > EPS, smooth C² extension for cv ≤ EPS\n");
            ...
            for (int t = 0; t < tcdis; t++) {
                ...
                sb.append(String.format("\n    HSP type t=%d: kb=%.12f, ms=%.12f, kb·ms=%.12f\n",
                ...
                for (int j = 0; j < lc[t]; j++) {
                    ...
                    sb.append(String.format("      Cluster (t=%d, j=%d): mh=%.12f, prefix=kb·ms·mh=%.12f\n",
                    ...
                    for (int v = 0; v < nv; v++) {
                        ...
                        sb.append(String.format(
                                "        cv[%d]=%.12e, w=%d, cv·ln(cv)_eff=%.12e, partialS=%.12e\n",
            ...
            sb.append("      S_decomp = Σ_t Σ_j S(t,j)\n");
            ...
            sb.append(String.format("      |ΔS|          = %.12e\n", sDiff));
            sb.append(String.format("      G = H - T·S = %.6e - %.1f × %.6f = %.6e\n",
            ...
            if (pass) {
                sb.append("    ✓ PASS: Relative error < 1%\n");
                ...
                sb.append("    ✗ FAIL: Relative error > 1%\n");
            ...
            sb.append("\n  [Diagnostics]\n");
            sb.append(String.format("    KB coefficient sum: Σ kb = %.10f\n",
            ...
            sb.append("\n  ✗ ERROR during entropy calculation:\n");
            ...
            sb.append("\n  ⚠ Cannot validate entropy - returning FAIL\n");
            return false;
        }
    }
}
