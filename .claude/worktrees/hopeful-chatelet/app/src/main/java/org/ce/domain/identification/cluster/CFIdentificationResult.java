package org.ce.domain.identification.cluster;
 ...
 * <h2>Two stages</h2>
 * <ol>
 *   <li><b>Stage 2a â€” HSP CFs</b> ({@code disCFData})<br>
 ...
 *
 *   <li><b>Stage 2b â€” Phase CFs</b> ({@code ordCFData}, {@code groupedCFData})<br>
 ...
 * <ul>
 *   <li>{@code lcf[t][j]} â€” number of CFs for HSP cluster {@code t},
 *       ordered-phase cluster group {@code j}</li>
 *   <li>{@code tcf} â€” total number of CFs</li>
 *   <li>{@code nxcf} â€” number of point-cluster CFs (used for normalization)</li>
 *   <li>{@code mcf[t][j]} â€” multiplicity of each CF group</li>
 *   <li>{@code rcf[t][j]} â€” per-sublattice site counts for each CF</li>
 * </ul>
 *
 * <h2>Mathematica equivalents</h2>
 * <pre>
 * disCFData    â†â†’  disCFData  (from genClusCoordList[disMaxClusCoord, disSymOps, basisN])
 * ordCFData    â†â†’  ordCFData  (from transClusCoordList[disCFData, CFData, CFCoordList])
 * groupedCFData â†â†’ cfData    (from groupCFData[disClusData, disCFData, ordCFData, ...])
 * lcf          â†â†’  lcf        (from readLength[cfCoordList])
 * tcf          â†â†’  tcf        (Î£ lcf)
 * nxcf         â†â†’  nxcf       (Î£ lcf[tcdis])
 * mcf          â†â†’  mcf        (cfData[[2]])
 * rcf          â†â†’  rcf        (cfData[[4]])
 * tcfdis       â†â†’  tcfdis     (total HSP CFs)
 * </pre>
 *
 * @see CFIdentifier
 */
public class CFIdentificationResult {
    ...
    /** Total number of CFs: {@code Î£_{t,j} lcf[t][j]}. */
    private final int tcf;

    /** Number of point-cluster CFs: {@code Î£_j lcf[tcdis-1][j]}. */
    ...
    public void printDebug() {
        ...
        System.out.println("â•‘     CORRELATION FUNCTION IDENTIFICATION          â•‘");
        System.out.println("â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•");

        System.out.println("\nâ”€â”€ Stage 2a: HSP CFs â”€â”€");
        System.out.println("  tcfdis = " + tcfdis + "  (total HSP CFs, excl. empty)");

        System.out.println("\nâ”€â”€ Stage 2b: Grouped CFs (lcf table) â”€â”€");
