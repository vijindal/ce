 ...
 * <pre>
 *   Î¦(e) = Î _{k=0}^{sizeâˆ’1}  Ï†_{Î±(k)}( occ[siteIndices[k]] )
 * </pre>
 * <p>where {@code Î±(k)} is the basis-function index of the {@code k}-th site
 * in the orbit member (read from its {@code "sÎ±"} symbol), and
 * {@code Ï†_Î±} is evaluated by {@link SiteOperatorBasis}.</p>
 *
 ...
 * {@link org.ce.domain.identification.result.ClusCoordListResult#getOrbitList()
 * orbitList[clusterType][orbitMemberIndex]} to read the site symbols.</p>
 *
 * <h2>Energy formula</h2>
 * <pre>
 *   H = Î£_{e âˆˆ allEmbeddings}  ECI[e.type] Â· Î¦(e) / size(e)    [for size > 0]
 *       + ECI[empty] Â· Î¦(empty)                                  [for empty cluster]
 * </pre>
...
public final class LocalEnergyCalc {
    ...
    /**
     * Computes the cluster product {@code Î¦(e)} for embedding {@code e}
     ...
     *                sourced from {@link org.ce.domain.identification.result.ClusCoordListResult#getOrbitList()}
     * @return cluster product {@code Î¦(e)}
     */
    public static double clusterProduct(Embedding e,
                                         LatticeConfig config,
                                         List<List<org.ce.domain.identification.geometry.Cluster>> orbits) {
